...
<#   
.SYNOPSIS   
    An interactive menu allows a user to connect to a local or remote computer and remove a local stored profile. 
.DESCRIPTION 
    Presents an interactive menu for user to first make a connection to a remote or local machine.  After making a connection to the machine,  
    the user is presented with all of the local profiles and then is asked to make a selection of which profile to delete. This is only valid on Windows Vista OS and above for clients and Windows 2008 and above for server OS.           
.EXAMPLE  
Remove-ProfileWF
  
Description 
----------- 
Presents a text-based menu for the user to interactively remove a local profile on a local or remote machine.    
#> 
